!*****************************************************************************************
subroutine put_r2gto_sym_ortho_wgrad(parini,bc,reset,nat,rxyz,qat,gw,rgcut,xyz111,ngx,ngy,ngz,hgrid,rho,wgrad)
    ...
    real(8), intent(inout):: wgrad(ngx,ngy,ngz)
    ...
    wa_grad=f_malloc([1-nagx.to.ngx+nagx,1-nagy.to.ngy+nagy,1-nagz.to.ngz+nagz],id='wa_grad')
    ...
    do iat=1,nat
        ...
        facqiat=fac*qat(iat)
        do iz=-nbgz,nbgz
            ttz=(width_inv_hgz*iz-width_inv_zat)**2
            rhoz=facqiat*wz(iz)
            jz=iatoz+iz
            do iy=-nbgy,nbgy
                tty=(width_inv_hgy*iy-width_inv_yat)**2
                rhoyz=rhoz*wy(iy)
                jy=iatoy+iy
                do ix=mboundg(1,iy,iz),mboundg(2,iy,iz)
                    ttx=(width_inv_hgx*ix-width_inv_xat)**2
                    jx=iatox+ix
                    !write(*,'(5i5)') iat,iatox,ix,iatoy,iy
                    wa(jx,jy,jz)=wa(jx,jy,jz)+rhoyz*wx(ix)*(ttx+tty+ttz)
                    wa_grad(jx,jy,jz)=wa_grad(jx,jy,jz)+rhoyz*wx(ix)*(ttx+tty+ttz)*(2.d0*(ttx+tty+ttz)-5.d0)/width
                enddo
            enddo
        enddo
    enddo
    ...
    call charge_back_to_cell(ngx,ngy,ngz,nagx,nagy,nagz,ibcx,wa_grad,wgrad)
    !do iz=1,ngz
    !    do iy=1,ngy
    !        do ix=1,ngx
...
end subroutine put_r2gto_sym_ortho_wgrad
!*****************************************************************************************
subroutine put_r2gto_sym_ortho(parini,bc,reset,nat,rxyz,qat,gw,rgcut,xyz111,ngx,ngy,ngz,hgrid,rho)
    use mod_parini, only: typ_parini
    use dynamic_memory
    implicit none
    type(typ_parini), intent(in):: parini
    ...
    call f_free(mboundg)
    call f_release_routine()
end subroutine put_r2gto_sym_ortho
!*****************************************************************************************
