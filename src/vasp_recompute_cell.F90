program recompute_cell
implicit none
integer:: i,nline,iat,nat,n,k
real(8):: latvec(3,3),angbohr,scaling
character(250):: tmp_ch 
character(250):: all_line(50),line
real(8):: xred(3,5000)
angbohr=1.d0/0.52917720859d0
 ...
 read(12,*) latvec(:,3)
  ...
  xred=0.d0
  do while(.true.)
   read(12,'(a250)',end=97,err=97)all_line(nline)
   n = len_trim(all_line(nline))
   k = index(all_line(nline)(1:n),"Direct")
    if(k.ne.0) then
             do while(.true.)
             nat=nat+1 
             read(12,'(a250)',end=97,err=97) line
             read(line,*,err=97,end=97) xred(:,nat)          
             enddo
    endif
    nline=nline+1
  enddo  
  ...
  nat=nat-1
  close(12)
call fixcell_niggli(nat,latvec,xred)

open(unit=12,file="CONTCAR")
write(12,'(a)') trim(tmp_ch)
write(12,'(f20.10)') scaling
write(12,'(3(es25.15))') latvec(:,1)
write(12,'(3(es25.15))') latvec(:,2)
write(12,'(3(es25.15))') latvec(:,3)
do i=1,nline
   write(12,'(a)') trim(all_line(i))
enddo
do iat=1,nat
   write(12,'(3(es25.15))') xred(:,iat)
enddo
