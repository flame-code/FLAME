!*****************************************************************************************
subroutine put_gto_p_ortho_wgrad(parini,bc,reset,nat,rxyz,p,gw,rgcut,xyz111,ngx,ngy,ngz,hgrid,rho,wgrad)
    ...
    call f_routine(id='put_gto_p_ortho_wgrad')
    ...
    do iat=1,nat
        ...
        facqiat=fac !*qat(iat)
        do iz=-nbgz,nbgz
            ssz=width_inv_hgz*iz-width_inv_zat
            rhoz=facqiat*wz(iz)
            jz=iatoz+iz
            do iy=-nbgy,nbgy
                ssy=width_inv_hgy*iy-width_inv_yat
                rhoyz=rhoz*wy(iy)
                jy=iatoy+iy
                do ix=mboundg(1,iy,iz),mboundg(2,iy,iz)
                    ssx=width_inv_hgx*ix-width_inv_xat
                    jx=iatox+ix
                    ...
                    wa_grad(jx,jy,jz)=wa_grad(jx,jy,jz)+rhoyz*wx(ix)*(ttx+tty+ttz)*(2.d0*(ssx**2+ssy**2+ssz**2)-5.d0)/width
                enddo
            enddo
        enddo
    enddo
...
end subroutine put_gto_p_ortho_wgrad
!*****************************************************************************************
