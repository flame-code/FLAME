!*****************************************************************************************
subroutine put_gto_sym_ortho_wgrad(parini,bc,reset,nat,rxyz,qat,gw,rgcut,xyz111,ngx,ngy,ngz,hgrid,rho,wgrad)
    ...
    call f_routine(id='put_gto_sym_ortho_wgrad')
    ...
    do iat=1,nat
        ...
        facqiat=fac*qat(iat)
        do iz=-nbgz,nbgz
            ttz=width_inv_hgz*iz-width_inv_zat
            rhoz=facqiat*wz(iz)
            jz=iatoz+iz
            do iy=-nbgy,nbgy
                tty=width_inv_hgy*iy-width_inv_yat
                rhoyz=rhoz*wy(iy)
                jy=iatoy+iy
                do ix=mboundg(1,iy,iz),mboundg(2,iy,iz)
                    ttx=width_inv_hgx*ix-width_inv_xat
                    jx=iatox+ix
                    !write(*,'(5i5)') iat,iatox,ix,iatoy,iy
                    wa(jx,jy,jz)=wa(jx,jy,jz)+rhoyz*wx(ix)
                    wa_grad(jx,jy,jz)=wa_grad(jx,jy,jz)+rhoyz*wx(ix)*(2.d0*(ttx**2+tty**2+ttz**2)-3.d0)/width
                enddo
            enddo
        enddo
    enddo
...
end subroutine put_gto_sym_ortho_wgrad
!*****************************************************************************************
