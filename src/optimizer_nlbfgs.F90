...
! 
!          WARNING: VALID FOR ORIGINAL VERSION
...
!     ----------
!
      IF(paropt%IFLAG.EQ.0) GO TO 10
      GO TO (172,100) paropt%IFLAG
  10  ITER= 0
      IF(N.LE.0.OR.M.LE.0) GO TO 196
      IF(paropt%GTOL.LE.1.D-04) THEN
        IF(paropt%LP.GT.0) WRITE(paropt%LP,245)
        paropt%GTOL=9.D-01
      ENDIF
      NFUN= 1
      POINT= 0
      FINISH= .FALSE.
      IF(paropt%DIAGCO) THEN
         DO 30 I=1,N
...
!     
      ...
      MAXFEV= 20
!
      IF(paropt%IPRINT(1).GE.0) CALL NLB1(ITER,NFUN,GNORM,N,M,X,F,G,STP,FINISH,paropt)
!
!    --------------------
!     MAIN ITERATION LOOP
!    --------------------
...
!     ----------------------------------------------------
 ...
 170  W(I)=-G(I)
 172  CONTINUE
      CALL NMCSRCH(N,X,F,G,W(ISPT+POINT*N+1),STP,FTOL,MAXFEV,INFO,NFEV,DIAG,paropt)
      IF (INFO .EQ. -1) THEN
        paropt%IFLAG=1
        RETURN
      ENDIF
...
!
      ...
      !IF (GNORM/XNORM .LE. paropt%EPS) FINISH=.TRUE.
      call calmaxforcecomponent(N,G,fmax)
      if(fmax<paropt%fmaxtol) FINISH=.TRUE.
!
      IF(paropt%IPRINT(1).GE.0) CALL NLB1(ITER,NFUN,GNORM,N,M,X,F,G,STP,FINISH,paropt)
      IF (FINISH) THEN
         paropt%IFLAG=0
...
!
 190  paropt%IFLAG=-1
      IF(paropt%LP.GT.0) WRITE(paropt%LP,200) INFO
      RETURN
 195  paropt%IFLAG=-2
      IF(paropt%LP.GT.0) WRITE(paropt%LP,235) I
      RETURN
 196  paropt%IFLAG= -3
      IF(paropt%LP.GT.0) WRITE(paropt%LP,240)
!
!     FORMATS
!     -------
!
...
!
      IF (ITER.EQ.0)THEN
           WRITE(paropt%MP,10)
           WRITE(paropt%MP,20) N,M
           WRITE(paropt%MP,30)F,GNORM
                 IF (paropt%IPRINT(2).GE.1)THEN
                     WRITE(paropt%MP,40)
                     WRITE(paropt%MP,50) (X(I),I=1,N)
                     WRITE(paropt%MP,60)
                     WRITE(paropt%MP,50) (G(I),I=1,N)
                  ENDIF
           WRITE(paropt%MP,10)
           WRITE(paropt%MP,70)
      ELSE
          IF ((paropt%IPRINT(1).EQ.0).AND.(ITER.NE.1.AND..NOT.FINISH))RETURN
              IF (paropt%IPRINT(1).NE.0)THEN
                   IF(MOD(ITER-1,paropt%IPRINT(1)).EQ.0.OR.FINISH)THEN
                         IF(paropt%IPRINT(2).GT.1.AND.ITER.GT.1) WRITE(paropt%MP,70)
                         WRITE(paropt%MP,80) 'MIN ',ITER,NFUN,F,GNORM,STP
                   ELSE
                         RETURN
                   ENDIF
              ELSE
                   IF( paropt%IPRINT(2).GT.1.AND.FINISH) WRITE(paropt%MP,70)
                   WRITE(paropt%MP,80) 'MIN ',ITER,NFUN,F,GNORM,STP
              ENDIF
              IF (paropt%IPRINT(2).EQ.2.OR.paropt%IPRINT(2).EQ.3)THEN
                    IF (FINISH)THEN
                        WRITE(paropt%MP,90)
                    ELSE
                        WRITE(paropt%MP,40)
                    ENDIF
                      WRITE(paropt%MP,50)(X(I),I=1,N)
                  IF (paropt%IPRINT(2).EQ.3)THEN
                      WRITE(paropt%MP,60)
                      WRITE(paropt%MP,50)(G(I),I=1,N)
                  ENDIF
              ENDIF
            IF (FINISH) WRITE(paropt%MP,100)
      ENDIF
!
 10   FORMAT('*************************************************')
...
!     80   FORMAT(2(I4,1X),3X,3(1PD10.3,2X))
 80   FORMAT(a,2i5,1es24.15,1es19.10,1es14.5)
...
!
      IF (N .LE. 0 .OR. STP .LE. ZERO .OR. FTOL .LT. ZERO .OR. &
         paropt%GTOL .LT. ZERO .OR. paropt%XTOL .LT. ZERO .OR. paropt%STPMIN .LT. ZERO &
         .OR. paropt%STPMAX .LT. paropt%STPMIN .OR. MAXFEV .LE. 0) RETURN
!
...
!
      ...
      FINIT = F
      DGTEST = FTOL*DGINIT
      WIDTH = paropt%STPMAX - paropt%STPMIN
      WIDTH1 = WIDTH/P5
      DO 20 J = 1, N
         WA(J) = X(J)
   20    CONTINUE
...
!
         STP = MAX(STP,paropt%STPMIN)
         STP = MIN(STP,paropt%STPMAX)
!
...
!
         IF ((BRACKT .AND. (STP .LE. STMIN .OR. STP .GE. STMAX)) &
            ...
            .OR. (BRACKT .AND. STMAX-STMIN .LE. paropt%XTOL*STMAX)) STP = STX
!
...
!
         ...
         IF (STP .EQ. paropt%STPMAX .AND. &
             F .LE. FTEST1 .AND. DG .LE. DGTEST) INFO = 5
         IF (STP .EQ. paropt%STPMIN .AND. &
             (F .GT. FTEST1 .OR. DG .GE. DGTEST)) INFO = 4
         IF (NFEV .GE. MAXFEV) INFO = 3
         IF (BRACKT .AND. STMAX-STMIN .LE. paropt%XTOL*STMAX) INFO = 2
         IF (F .LE. FTEST1 .AND. ABS(DG) .LE. paropt%GTOL*(-DGINIT)) INFO = 1
!
...
!
         IF (STAGE1 .AND. F .LE. FTEST1 .AND. &
             DG .GE. MIN(FTOL,paropt%GTOL)*DGINIT) STAGE1 = .FALSE.
!
...
!
            CALL NMCSTEP(STX,FXM,DGXM,STY,FYM,DGYM,STP,FM,DGM,BRACKT,STMIN,STMAX,INFOC)
!
...
!           AND TO COMPUTE THE NEW STEP.
!
            CALL NMCSTEP(STX,FX,DGX,STY,FY,DGY,STP,F,DG,BRACKT,STMIN,STMAX,INFOC)
            END IF
!
