...
subroutine test_optimize_basis_functions
    ...
    errmax=0.d0
    errmax=max(errmax,abs(fitpar%gwc_s1(1)-( 1.001708720898542d+00)))
    errmax=max(errmax,abs(fitpar%gwc_s1(2)-( 1.001747931975960d+00)))
    errmax=max(errmax,abs(fitpar%bc_s1(1) -( 9.994304263671525d-01)))
    errmax=max(errmax,abs(fitpar%bc_s1(2) -( 9.994173560080135d-01)))
    errmax=max(errmax,abs(fitpar%gwv_s1(1)-( 1.055539484040235d+00)))
    errmax=max(errmax,abs(fitpar%gwv_s1(2)-( 1.025499462449259d+00)))
    errmax=max(errmax,abs(fitpar%bv_s1(1) -(-1.212468121717266d+00)))
    errmax=max(errmax,abs(fitpar%bv_s1(2) -( 5.509390168192465d-01)))
