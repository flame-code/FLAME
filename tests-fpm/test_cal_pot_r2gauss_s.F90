...
subroutine test_cal_pot_r2gauss_s
    ...
    if(errmax<1.d-7) then
        write(output_unit,'(2a)') green_passed,' in test_cal_pot_r2gauss_s: errmax'
    else
        write(error_unit,'(2a,es14.5)') red_failed,' in test_cal_pot_r2gauss_s: errmax=',errmax
        call exit(1)
    end if
    gw=gw+1.d-5
    call cal_pot_r2gauss_s(parini,poisson,pot_t,.true.,xyz,gw,q,vgrad_t)
    ...
    if (errmax_vgrad<1.d-6) then
        write(output_unit,'(2a)') green_passed,' in test_cal_pot_r2gauss_s: errmax_vgrad'
    else
        write(error_unit,'(2a,es14.5)') red_failed,' in test_cal_pot_r2gauss_s: errmax_vgrad=',errmax_vgrad
        call exit(1)
    end if
