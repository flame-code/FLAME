!*****************************************************************************************
subroutine test_cal_pot_gauss_p
    use iso_fortran_env, only: error_unit, output_unit
    use mod_fit_bf_cent2, only: cal_pot_gauss_p
    ...
    implicit none
    !integer:: nx, ny, nz
    real(8):: gw, p(3), xyz(3), ener, gausswidth(1), errmax
    real(8):: errmax_vgrad
    !logical::
    ...
    call cal_pot_gauss_p(parini,poisson,pot,.true.,xyz,gw,p,vgrad)
    ...
    call put_gto_p_ortho(parini,poisson%bc,.true.,1,atoms%ratp(1,1),p,gausswidth(1), &
...
!        write(83,'(3f8.3,3es19.10)') x,y,z,poisson%pot(ix,iy,iz),pot(ix,iy,iz),poisson%rho(ix,iy,iz)
!    enddo
!    enddo
!    enddo
    ...
    if(errmax<1.d-7) then
        write(output_unit,'(2a)') green_passed,' in test_cal_pot_gauss_p: errmax'
    else
        write(error_unit,'(2a,es14.5)') red_failed,' in test_cal_pot_gauss_p: errmax=',errmax
        call exit(1)
    end if
    gw=gw+1.d-5
    call cal_pot_gauss_p(parini,poisson,pot_t,.true.,xyz,gw,p,vgrad_t)
    ...
    if (errmax_vgrad<1.d-6) then
        write(output_unit,'(2a)') green_passed,' in test_cal_pot_gauss_p: errmax_vgrad'
    else
        write(error_unit,'(2a,es14.5)') red_failed,' in test_cal_pot_gauss_p: errmax_vgrad=',errmax_vgrad
        call exit(1)
    end if
    ...
    deallocate(pot)
    deallocate(vgrad)
end subroutine test_cal_pot_gauss_p
!*****************************************************************************************
